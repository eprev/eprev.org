      ...
      if (settings.rewrite) {
        ...
        files.forEach(file => {
          if (!mapping[file] && match.test(file)) {
            ...
            if (/\.md$/.test(file)) {
              const source = fs.readFileSync(path.join(config.public, file), { encoding: 'utf8' });
              const { content, meta } = markdown(source, { baseUrl: config.site.url });
              collections[name].push(Object.assign({}, meta, {
                url: mapping[file],
                content,
              }));
            }
