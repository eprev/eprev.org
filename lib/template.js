...
const render = (exports.render = function(name, context) {
  const tmpl = fs.readFileSync(path.join(templateDir, name + '.js'), 'utf8');
  return renderString(tmpl, context);
});

const renderFile = (exports.renderFile = function(filename, context) {
  const tmpl = fs.readFileSync(path.join(config.public, filename), 'utf8');
  return renderString(tmpl, context);
});
