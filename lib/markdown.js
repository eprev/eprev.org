        ...
        items.forEach(item => {
          if (item) {
            ...
            for (let i = listItemIndex + 1; i < this._tokens.length; i++) {
              if (/(paragraph|heading|list|block_quote)_start|block_code/.test(this._tokens[i].type)) {
                blockNo++;
              }
            }
            if (blockNo === 1) {
...
const render = (module.exports.render = function(tokens, { baseUrl = '' }) {
  function makeUrl(s) {
  ...
  return tokens
    .map(token => {
      const className = token.className ? ` class=${token.className}` : '';
      switch (token.type) {
        case 'thematic_break':
          return '<hr>';
        case 'text':
        ...
        case 'block_code':
          ...
          return `<figure class="highlight${token.className || ''}"><code><div class="scrollable"><pre>` + value + `</pre></div></code></figure>`;
        case 'em_start':
          return `<em${className}>`;
        case 'em_end':
          return '</em>';
        ...
        case 'strong_end':
          return '</strong>';
        case 'code':
          return `<code${className}>${token.value}</code>`;
        case 'link_start':
          return (
        ...
        case 'image':
          return (
            `<img src="${makeUrl(token.src)}" alt="${token.alt}"${className}` +
            (token.title ? ` title=${token.title}` : '') +
            '>'
          );
        case 'comment':
