module.exports = function(text, options) {
  ...
  let meta = {};
  if (tokens.length && tokens[0].type === 'comment') {
    meta = tokens[0].value.split(/\n+(?=\S)/g).reduce((acc, line) => {
    ...
    if (!meta.title) {
      const headingIndex = tokens.findIndex(t => t.type === 'heading_start');
      if (headingIndex) {
        const headingToken = tokens[headingIndex + 1];
  ...
  return tokens
    .map(token => {
      switch (token.type) {
        ...
        case 'image':
          return (
            `<img src="${makeUrl(token.src)}" alt="${token.alt}"` +
            (token.title ? ` title=${token.title}` : '') +
            '>'
          );
        case 'comment':
