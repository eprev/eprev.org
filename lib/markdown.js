  ...
  _blockToken(source) {
    while (source) {
      ...
      if ((match = block.list.exec(source))) {
        console.log(match);
        const [gstr, indent, marker] = match;
        const style = ['*', '-', '+'].includes(marker) ? 'bullet': 'ordered';
        this._tokens.push({
          type: 'list_start',
          style,
        });
        const items = gstr.split(/^[*+-]|[0-9][.)]\s*/mg);
        ...
        //   '(?:' + // match line:
        //     '(?:' + // non blank:
        //       '(?:[*+-]|[0-9][.)]) +' + // line starting with a list marker followed by any numer of spaces
        //       '|' +
        //       ' {' + indent.length + ',}[^ ]' + // or line indented as much as the text in the first item
        //     ')' +
        //     '[^\\n]*\\n?' +
        //     '|' +
        //     '\\n' + // blank
        ...
        items.forEach(item => {
          if (item) {
            ...
            this._blockToken(
              item.replace(new RegExp('^ {' + indent.length + '}', 'gm'), '').trimRight()
            );
